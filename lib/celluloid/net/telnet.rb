...
module Net
  ...
  class Telnet
    ...
    #           Timeout::Error to be raised.  The default value is 10 seconds.
    ...
    def initialize(options) # :yield: mesg
      ...
      if @options.has_key?("Proxy")
        ...
        begin
          ...
          raise Timeout::Error, "timed out while opening a connection to the host"
        rescue
    ...
    def wait_socket_readable(sock, time_out)
      begin
        wait_readable(time_out) do
          sock.wait_readable
          true
        end
       rescue Timeout::Error
    ...
    def waitfor(options) # :yield: recvdata
      ...
      rest = ''
      wait_readable(time_out) do
        until(prompt === line and ((!waittime || waittime ==0) or not wait_socket_readable(@sock, waittime)))
          begin
            @sock.wait_readable
